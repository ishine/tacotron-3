...
hparams = tf.contrib.training.HParams(
	...
	num_freq = 513, #1025, # (= n_fft / 2 + 1) only used when adding linear spectrograms post processing network #zhouyqzhouyq 20190312
	...
	#Mel spectrogram
	n_fft = 1024, #2048, #Extra window size is filled with 0 paddings to match this parameter #zhouyqzhouyq 20190312
	hop_size = 200, #275, #For 22050Hz, 275 ~= 12.5 ms (0.0125 * sample_rate)
	win_size = 800, #1100, #For 22050Hz, 1100 ~= 50 ms (If None, win_size = n_fft) (0.05 * sample_rate)
	sample_rate = 16000, #22050, #22050 Hz (corresponding to ljspeech dataset) (sox --i <filename>)
	...
	upsample_scales = [10, 20],# [11, 25], #prod(upsample_scales) should be equal to hop_size #zhouyqzhouyq 20190312
	...
	tacotron_batch_size = 8, #32, #number of training samples on each training steps #zhouyqzhouyq 20190319
	...
	#train/test split ratios, mini-batches sizes
	wavenet_batch_size = 1,# 8, #batch size used to train wavenet. #zhouyqzhouyq 20190307
